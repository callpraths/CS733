...
void test_ppp_gaty(char const * A_file, char const * B_file, char const * C_file)
{
  ...
  cs *  R = cs_spalloc(A->n, 1, (A->p[A->n] < A->n)? A->p[A->n] : A->n, 1, 0); /* allocate result*/
...
int main(int argc, char ** argv)
{
  ...
  if(argc == 3)
    test_ppp_gaty(argv[1], argv[2], NULL);
  else
    test_ppp_gaty(argv[1], argv[2], argv[3]);
  return 0;
}
